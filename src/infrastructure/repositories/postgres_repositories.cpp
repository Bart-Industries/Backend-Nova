...
{
    const auto result = db_->execSqlSync("select id, name, email, password_hash, role, is_active from users where email = $1 limit 1", email);
    if (result.empty())
        return std::nullopt;
...
{
    const auto result = db_->execSqlSync("select id, name, email, password_hash, role, is_active from users where id = $1 limit 1", id);
    if (result.empty())
        return std::nullopt;
    return mapUser(result[0]);
}
std::vector<domain::identity::User> PostgresUserRepository::listActive()
{
    ...
    for (const auto &row : db_->execSqlSync("select id, name, email, password_hash, role, is_active from users where is_active = true order by id asc"))
...
{
    const auto result = db_->execSqlSync("insert into users (name, email, password_hash, role, is_active) values ($1, $2, $3, $4, $5) returning id, name, email, password_hash, role, is_active",
                                         user.name,
                                         user.email,
                                         user.passwordHash,
                                         domain::toString(user.role),
                                         user.isActive);
    return mapUser(result[0]);
}
...
std::vector<domain::menu::Category> PostgresCategoryRepository::listAll(bool onlyActive)
{
    ...
    const auto sql = onlyActive ? "select id, name, description, is_active from categories where is_active = true order by id asc"
...
{
    const auto result = db_->execSqlSync("insert into categories (name, description, is_active) values ($1, $2, $3) returning id, name, description, is_active",
                                         category.name,
...
{
    const auto result = db_->execSqlSync("update categories set name = $1, description = $2 where id = $3 returning id, name, description, is_active",
                                         category.name,
                                         category.description,
                                         id);
    return mapCategory(result[0]);
...
std::vector<domain::orders::OrderItem> PostgresOrderRepository::loadItems(std::int64_t orderId)
{
    ...
    for (const auto &row : db_->execSqlSync("select oi.id, oi.product_id, p.name as product_name, oi.quantity, oi.unit_price, oi.notes, oi.status from order_items oi inner join products p on p.id = oi.product_id where oi.order_id = $1 order by oi.id asc", orderId))
    {
...
domain::orders::Order PostgresOrderRepository::create(const domain::orders::Order &order)
{
    ...
    const auto orderResult = transaction->execSqlSync("insert into orders (table_id, customer_name, status, total) values ($1, $2, $3, $4) returning id, table_id, $5 as table_number, customer_name, status, total, created_at",
                                                      order.tableId,
                                                      order.customerName,
                                                      domain::toString(order.status),
                                                      order.total,
                                                      order.tableNumber);
    auto created = mapOrderBase(orderResult[0]);
    for (const auto &item : order.items)
    {
        const auto itemResult = transaction->execSqlSync("insert into order_items (order_id, product_id, quantity, unit_price, notes, status) values ($1, $2, $3, $4, $5, $6) returning id",
                                                         created.id,
                                                         item.productId,
                                                         item.quantity,
                                                         item.unitPrice,
